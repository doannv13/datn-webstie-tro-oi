                ...
                <table class="table table-centered mb-0" id="tech-companies-1">
                    ...
                    <tbody class="align-items-center p-4">
                        ...
                        <tr class="al">
                            ...
                            <td>
                                <select class="form-select mb-3 toggle-class statusSelect" data-id="{{ $value->id }}" name="status" id="statusSelect">
                                    <option value="pendding" {{ $value->status == 'pendding' ? 'selected' : false }}>
                                        Chờ xử lý
                                    </option>
                            ...
                            <td class="">
                                <div class="d-flex">
                                    ...
                                    <button class="btn btn-success my-1" style="font-size: 13px" data-bs-toggle="modal" data-bs-target="#exampleModalToggle{{ $value->id }}">
                                    ...
                                    <form action="{{ route('admin-room-posts.destroy', $value->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button id="deleteButton{{ $value->id }}" type="submit" class="btn btn-danger m-1 delete-button" style="width: 45px;" disabled onclick="return confirm('Bạn có muốn thêm vào thùng rác')">
                                    ...
                                    <form action="{{ route('admin-room-posts.destroy', $value->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button id="deleteButton{{ $value->id }}" type="submit" class="btn btn-danger m-1 delete-button" style="width: 45px;" onclick="return confirm('Bạn có muốn thêm vào thùng rác')">
                                            <i class="fa-solid fa-trash fs-5"></i>
                                        </button>
                                    </form>
                                    @endif
                                    <a href="{{ route('admin-room-posts.edit', $value->id) }}">
                                        <button type="submit" class="btn btn-primary text-center my-1" style="width: 45px;"> <!-- Đặt kích thước cố định là 100px -->
